...
const TestPage = () => {
  ...
  const handleSubmit = async () => {
    ...
    try {
      const res = await fetch("http://localhost:5000/create-test", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
  ...
  return (
    <div className="min-h-screen bg-slate-50 py-10 px-4 flex flex-col items-center">
      ...
      <div className="w-full max-w-7xl mb-8">
        ...
        <p className="text-gray-500 mt-1">Test Code: <span className="font-mono font-semibold text-indigo-600">{testCode}</span></p>
      </div>

      <div className="flex w-full max-w-7xl gap-6">

        {/* LEFT - NAVIGATOR */}
        <div className="w-1/3 card p-6 h-fit sticky top-6">
          <h2 className="text-sm font-bold text-gray-400 uppercase tracking-wider mb-4">Questions</h2>
          ...
          <div className="grid grid-cols-5 gap-2">
              ...
              <button
                ...
                className={`h-11 rounded-xl text-sm font-semibold transition-all duration-200
                  ${index === currentQuestionIndex 
                    ...
                    : "bg-gray-50 text-gray-600 border border-gray-200 hover:bg-indigo-50 hover:text-indigo-600 hover:border-indigo-200"}
                `}
              >
                {index + 1}
              </button>
            ...
            <button
              onClick={() => handleQuestionClick(questions.length)}
              className={`h-11 rounded-xl text-lg font-medium transition-all duration-200 border
                ${currentQuestionIndex === questions.length 
        ...
        <div className="flex-1 card p-8">
          ...
          <div className="flex items-center gap-3 mb-8">
            ...
            <h2 className="text-xl font-bold text-gray-900">
              {currentQuestionIndex < questions.length ? 'Edit Question' : 'New Question'}
            </h2>
          </div>
            ...
            <div className="flex gap-3">
              ...
              {questions.length > 0 && (
                <button
                  ...
                  className="inline-flex items-center justify-center rounded-xl bg-white border border-rose-200 px-5 py-2.5 text-sm font-semibold text-rose-600 hover:bg-rose-50 transition-all"
                >
                  Delete
                </button>
              )}
              ...
              {questions.length > 0 && (
                <button
                  onClick={handleSubmit}
                  className="btn-primary !py-2.5 !px-8 ml-1"
                >
                  Publish Test
                </button>
              )}
